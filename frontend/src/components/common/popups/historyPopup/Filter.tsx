...
const Filter = ({sortRespect, setSortRespect, selectedDate1, selectedDate2, setSelectedDates}: FilterInterface) => {
  return (
    <div className="w-full flex gap-3">
      <button className="h-10 w-full cursor-pointer px-4 bg-purple-400 rounded-lg" onClick={e => {
        switch (sortRespect) {
          case 'Все': {
      ...
      <div className="flex flex-row items-center gap-3">
        ...
        <input
          ...
          max={new Date().toLocaleDateString().split('.').reverse().join('-')}
        ></input>
        <button disabled={!(selectedDate1.length > 0 || selectedDate2.length > 0)} className="self-stretch bg-[--respect-purple-dark] cursor-pointer rounded-md px-[13px] disabled:cursor-not-allowed disabled:text-gray-400" onClick={(e) => {
